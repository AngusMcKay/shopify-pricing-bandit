...
(function () {
  ...
  (function injectSuppressCSS() {
    try {
      ...
      style.textContent =
        // Direct suppression of inputs, selects, labels, and IDs referencing _pm_price.
        '[name="_pm_price"],[name="options[_pm_price]"],[id*="_pm_price"],[for*="_pm_price"]' +
        '{display:none!important}' +
        // Container suppression via :has() — hides the entire option group block.
        // Excludes <form> itself to avoid hiding the whole add-to-cart form.
        'fieldset:has([name*="_pm_price"]),' +
        'li:has([name*="_pm_price"]),' +
        'div:has([name*="_pm_price"]):not(form),' +
  ...
  function hideOptionContainer(startEl) {
    try {
      ...
      for (var depth = 0; depth < 7; depth++) {
        ...
        // If this node's siblings also contain option selectors (select, radio,
        // checkbox), then `node` is one option group in a list — hide it.
        var children = Array.prototype.slice.call(parent.children);
        var siblingHasOptions = children.some(function (sib) {
          return sib !== node &&
            sib.querySelector('select, input[type="radio"], input[type="checkbox"]') !== null;
        });
        if (siblingHasOptions) {
          node.style.setProperty('display', 'none', 'important');
          return;
        }
        node = parent;
      }
      // Fallback: hide the element we started from.
